...
pub fn render(id: std.builtin.SourceLocation, args: Node) !void {
    ...
    const html = generateHtmlFile(id) catch |e| switch(e) {
        RenderError.InvalidPageFilePath => {std.debug.print("invalid file path: {s}. move below src/pages/**", .{id.file});return;},
        else => return,
    };
    ...
    const head_file = std.fs.cwd().openFile("zig-out/html/tmp.html", .{.mode = .read_write}) catch {
        const tmp = try std.fs.cwd().createFile("zig-out/html/tmp.html", .{.read=true});
    ...
    try std.fs.cwd().deleteFile("zig-out/html/tmp.html");
}
            ...
            switch (plane.tag) {
                .empty, .raw => {
                    ...
                    if(std.mem.eql(u8, tag, "head")) {
                        ...
                        var head_output = try std.fs.cwd().createFile("zig-out/html/tmp.html", .{.read=true});
